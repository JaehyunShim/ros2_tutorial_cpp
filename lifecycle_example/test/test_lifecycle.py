...
def generate_test_description():
    talker_node = launch_ros.actions.LifecycleNode(
        package='lifecycle_example', executable='lifecycle_publisher',
        name='lifecycle_talker', namespace='', output='screen'
    )
    listener_node = launch_ros.actions.LifecycleNode(
        package='lifecycle_example', executable='lifecycle_subscriber',
        name='lifecycle_listener', output='screen'
    )
    return launch.LaunchDescription([
        ...
        launch.actions.RegisterEventHandler(
            launch_ros.event_handlers.OnStateTransition(
                target_lifecycle_node=listener_node, start_state='activating', goal_state='active',
                entities=[
                    launch.actions.TimerAction(period=5.0, actions=[
                        launch.actions.EmitEvent(event=launch_ros.events.lifecycle.ChangeState(
                            lifecycle_node_matcher=launch.events.matches_action(listener_node),
...
class TestLifecyclePubSub(unittest.TestCase):
    ...
    def test_talker_lifecycle(self, proc_info, proc_output, talker_node, listener_node):
        ...
        proc_output.assertWaitFor('on_deactivate() is called', process=talker_node, timeout=10)
        proc_output.assertWaitFor('on cleanup is called', process=talker_node, timeout=5)
        proc_output.assertWaitFor('on shutdown is called', process=talker_node, timeout=5)

        proc_output.assertWaitFor('on_configure() is called', process=listener_node, timeout=5)
        proc_output.assertWaitFor('on_activate() is called', process=listener_node, timeout=5)
        proc_output.assertWaitFor('on_deactivate() is called', process=listener_node, timeout=10)
        proc_output.assertWaitFor('on cleanup is called', process=listener_node, timeout=5)
        proc_output.assertWaitFor('on shutdown is called', process=listener_node, timeout=5)
